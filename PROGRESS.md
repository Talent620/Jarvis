...
- [x] **Faza E — Odświeżenie modeli + brama OpenRouter** ✅ Zweryfikowano `TASK_MODELS`/registry: Groq już na **Llama 4 Scout** (proste/wizja) + **Kimi K2** (rozumowanie) z Fazy 4; Gemini 2.5, Claude Opus 4.8, OpenRouter free — aktualne wobec wiedzy modelu. **OpenRouter** potwierdzony jako (a) **brama failover** w `routeOrder` (ranga 60 — wchodzi za dostawcami wyższej rangi) oraz (b) **źródło salda/kosztów** (`openrouterBalance.ts` → panel 💸, Faza 6-safe). +3 testy bramy (obecność w katalogu, wejście do łańcucha, kolejność failover Anthropic→OpenRouter); router Scout/Kimi pokryty (8 testów). **792 zielone**; build OK. **Ograniczenie środowiska (uczciwie):** bez kluczy live nie da się odpytać żywych list modeli dostawców — NIE zgaduję nowych ID „w ciemno" (ryzyko zerwania działających konfiguracji); ID zweryfikowane wobec stanu wiedzy, realne re-potwierdzenie wymaga Twoich kluczy.
- [x] **Faza F — Utwardzenie pod sprzedaż (STOP-and-ASK)** ✅ **Wdrożone bezpieczne (wcześniejsze sesje + potwierdzone):** szyfrowanie kluczy BYOK w spoczynku (`secretsVault.ts` + `secretsAtRest`, AES-256, migracja wsteczna); BFF fail-closed **opt-in** (`REQUIRE_APP_TOKEN`); auth `x-app-token` na `/v1/search`/`/v1/embed`/`/anthropic`/`/gemini`/`/openai`/`/passthrough` gdy `APP_TOKEN` ustawiony; anty-SSRF `blocksCloudMetadata` na `/passthrough` + rate-limit; CORS Electrona zawężony (zero-regresji). **Pozycje ryzykowne — ODŁOŻONE decyzją właściciela (2026-06-19, po STOP-and-ASK):** (1) `admin.ts` sekret na numerze telefonu → „Zostaw na razie"; (2) keystore/podpis EXE/`release.yml`→tagi → „Nie ruszaj" (nieodwracalne wobec Play + zerwałoby rolling `latest`); (3) BFF fail-closed domyślnie + app-token na `/v1/sync` → „Zostaw opt-in" (domyślne wymaganie zablokowałoby worker bez `APP_TOKEN`). Brak zmian kodu w tej fazie — świadoma decyzja, nie pominięcie.
- [x] **Faza G — Samokontrola + RELEASE_NOTES.md** ✅ Pełny self-check: `tsc` czysty, `npm run build` OK (główny bundle ~562 KB; 4 workery on-device wydzielone po ~0.85 KB — ciężkie biblioteki z CDN w runtime), **792 testy zielone**, skan sekretów czysty. Przegląd edge-case'ów: brak WebGPU → chmura; brak sieci → łańcuch failover + czytelny błąd; przerwane pobieranie wag → timeout → fallback; brak kredytów → rotacja kluczy/dostawców; odmowa mic/kamery → czytelny komunikat — **wszystkie z fallbackiem**. `RELEASE_NOTES.md` rozszerzony o warstwę on-device (jak włączyć, wymagania WebGPU, ograniczenia, edge-case'y). Program on-device domknięty.

### Podsumowanie programu on-device
Dostarczono pełną, opcjonalną warstwę AI lokalnej (pamięć + mózg + głos) na WebGPU/WASM, jako rozszerzenie istniejącej architektury — **bez naruszenia ścieżki chmurowej**. Każda funkcja: capability-check + cichy fallback, inferencja w Web Workerze, wagi cache'owane, leniwe ładowanie z CDN (główny bundle praktycznie bez zmian). +60 testów (730→792). Świadomie odłożone (ryzyko/środowisko): refaktor uplinku Gemini Live na AudioWorklet/Silero VAD (ryzyko dla działającego endpointingu, brak testu audio); ryzykowne pozycje bezpieczeństwa Fazy F (decyzja właściciela); re-weryfikacja ID modeli wobec żywych list (wymaga kluczy).
